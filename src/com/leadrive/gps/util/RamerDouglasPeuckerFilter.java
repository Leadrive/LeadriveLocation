...
public class RamerDouglasPeuckerFilter {
     ...
     * @return the points of the simplified track
     */
    public static TrackPoint[] simplify(TrackPoint[] track, int start, int end,
            double epsilon) {
        ...
        int num = 0;
        for (int i = start; i <= end; ++i) {
        	sum += track[i].getScore();
        	num ++;
            if (keep[i]) {
                result[k] = track[i];
                result[k].setScore((int) ((float)sum/(float)num));
                ++k;
                num = 0;
                sum = 0;
            }
    ...
    private static void douglasPeucker(TrackPoint[] track, int first, int last,
